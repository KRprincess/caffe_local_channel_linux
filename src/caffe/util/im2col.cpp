...
void im2col_cpu(const Dtype* data_im, const int channels,
	...
	const int output_w = (width + 2 * pad_w -
		(dilation_w * (kernel_w - 1) + 1)) / stride_w + 1;
	const int channel_size = height * width;
	for (int channel = 0; channel<channels + 2 * pad_c; channel++) {//for C
		int input_channel = -pad_c + channel;//add
		for (int kernel_row = 0; kernel_row < kernel_h; kernel_row++) {//for K
			for (int kernel_col = 0; kernel_col < kernel_w; kernel_col++) {//for K
				if (!is_a_ge_zero_and_a_lt_b(input_channel, channels)){
					for (int output_rows = output_h; output_rows; output_rows--) {
						for (int output_col = output_w; output_col; output_col--){
							*(data_col++) = 0;
						}
					...
					int input_row = -pad_h + kernel_row * dilation_h;
					for (int output_rows = output_h; output_rows; output_rows--) {//for H
						if (!is_a_ge_zero_and_a_lt_b(input_row, height)) {
							for (int output_cols = output_w; output_cols; output_cols--) {//for W
								*(data_col++) = 0;
							}//end for W
						}
						else {
							int input_col = -pad_w + kernel_col * dilation_w;
							for (int output_col = output_w; output_col; output_col--) {//for W
								if (is_a_ge_zero_and_a_lt_b(input_col, width)) {
									*(data_col++) = data_im[input_row * width + input_col];
								}
								else {
...
void col2im_cpu(const Dtype* data_col, const int channels,
	...
	data_col += pad_c*kernel_h*kernel_w*output_h*output_w;
	for (int channel = pad_c; channel<(channels + pad_c); channel++) {
		for (int kernel_row = 0; kernel_row < kernel_h; kernel_row++) {
			for (int kernel_col = 0; kernel_col < kernel_w; kernel_col++) {
				int input_row = -pad_h + kernel_row * dilation_h;
				for (int output_rows = output_h; output_rows; output_rows--) {
